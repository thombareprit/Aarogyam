...
class TestLangchain:
    ...
    @pytest.mark.parametrize("model_type", [model.value for model in ModelTypes])
    def test_03_sequential_chain(self, setup_class, model_type):
    ...
    @pytest.mark.parametrize("model_type", [model.value for model in ModelTypes])
    def test_05_transformation_chain(self, setup_class, model_type):
    ...
    @pytest.mark.parametrize("model_type", [model.value for model in ModelTypes])
    def test_07_conversation_buffer_memory_chain(self, setup_class, model_type):
        model = Model(
            model_id=model_type,
            credentials=self.wml_credentials,
            project_id=self.project_id)
    ...
    @pytest.mark.parametrize("model_type", [model.value for model in ModelTypes if model.value not in ['google/flan-t5-xxl', 'google/flan-ul2', 'ibm/granite-13b-chat-v1']])
    ...
    @pytest.mark.parametrize("model_type", [model.value for model in ModelTypes])
    def test_09_to_langchain_wrapper(self, setup_class, model_type):
        prompt_template = "What color is the {flower}?\n"
        model = Model(
            model_id=model_type,
            credentials=self.wml_credentials,
